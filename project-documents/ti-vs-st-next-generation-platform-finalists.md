...
- **F28P55X** — the TMS320F28P550SJ has one true buffered 12-bit DAC (a resistor-string converter with an on-chip output buffer, "DACA"). The datasheet pin table places DACA_OUT on analog pin **A0**, and the LaunchPad user guide states that *all* analog (ADCIN) pins are routed to the BoosterPack headers — so the buffered DAC arrives at the header as a genuine DC-accurate analog output. No filtering, no PWM anywhere in the chain. (One nuance for the custom-board schematic: A0 is shared between DACA_OUT and ADC input A0, so that channel is one or the other.)
...
| On-chip DAC (true, buffered) | 1× 12-bit (DACA_OUT on pin A0, at header) | 1× 12-bit, **2 channels** (PA4/PA5, at headers) |
| Encoders | **3× eQEP: HW index, HW 1/T velocity capture, error flags; 2 connectors on board** | Encoder mode on 2× 32-bit GP timers; index via EXTI ISR; T-method velocity only |
| PWM | **24 ePWM (12 high-res, 150 ps)** | Advanced MC timer + GP timers |
| RT determinism | Sub-µs interrupts; purpose-built RT MCU | Sub-µs interrupts |
...
| C libraries | C2000Ware: driverlib + examples **for this board**, DCL control library, IQmath, FreeRTOS demos | CubeH5: HAL/LL + CMSIS-DSP; no H533RE example projects; ThreadX-first middleware |
...
- **Encoder count:** how many simultaneous encoders does the heaviest lab use? TI handles 3 in dedicated hardware; the H533 handles 2 on general-purpose timers. If the answer is ever "3," the decision makes itself.
...
- [TI — TMS320F28P55x datasheet (DACA_OUT on pin A0)](https://www.ti.com/lit/ds/symlink/tms320f28p550sj.pdf)
- [TI — LAUNCHXL-F28P55X user guide SPRUJC0 (§2.1.3.8 PWM-DAC; ADCIN routing to headers)](https://www.ti.com/lit/ug/sprujc0a/sprujc0a.pdf)
...
- [TI — C2000Ware SDK (F28P551x driverlib, examples, FreeRTOS demos, libraries)](https://www.ti.com/tool/C2000WARE)
- [TI — Digital Control Library user's guide (SPRUID3)](https://www.ti.com/lit/ug/spruid3/spruid3.pdf)
- [ST — STM32CubeH5 on GitHub (HAL/LL, CMSIS, ThreadX middleware; example boards H563ZI/H503RB/H573I-DK)](https://github.com/STMicroelectronics/STM32CubeH5)
