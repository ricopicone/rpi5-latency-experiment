...
Board comparison—internal—August 2026 (rev 7)
...
| macOS on Apple Silicon | **x86 only; Rosetta 2 required.** Native build promised by end of 2026 (see section) | **Native arm64** since Feb 2026 |
...
**Where TI stands (as of August 2026): not yet native, but committed with a live deadline.** The current release, CCS 21.0.0 (June 15, 2026), ships a single macOS installer—`CCS_21.0.0.00014_mac_x86.dmg`. There is no arm64 build. TI has delivered part of the work: the 20.4.0 release notes state that the Theia IDE component is now compiled for Arm, but immediately qualify that "the backend components are still x86 based, so Rosetta is still required." The editor shell went native; the compiler, debug server, and XDS110 driver stack did not, and those are the harder half.

The commitment is recent and specific. In an E2E thread opened in **July 2026**—one month before this revision—a TI engineer answered the Apple Silicon question with: "Yes. Work is already in progress for this. A version will be available before the end of the year, if not sooner." That places the promised delivery inside the remaining months of **2026**, and as of this writing the deadline has not passed. Read plainly, TI's own stated schedule beats Apple's macOS 28 cutoff by roughly a year. **This is a commitment to track, not one to discount**—see the watch item in the open questions.
...
**Assessment.** ST holds a real advantage today, and this is the only place in the comparison where the toolchains genuinely diverge. It should not reverse the recommendation: TI has committed publicly and recently, has completed the frontend half of the work, has named a deadline that is still open, and has a large education business that would be badly damaged by CCS failing on student Macs. The residual risk is schedule slip, not direction—and even a slipped TI release has roughly a year of margin before Intel apps stop running altogether. Unlike most items in this document, though, it is outside our control, so it gets an explicit watch item rather than a footnote.
...
- **WATCH—native arm64 CCS for Apple Silicon.** TI's public commitment (E2E, July 2026) is a native macOS build "before the end of the year, if not sooner." That deadline is still open as of this revision, so nothing is broken yet—but it is the one dependency in this comparison we neither control nor have a contract on. Checkpoints: **(1)** each CCS release, confirm whether the macOS download list has gained an `arm64` DMG alongside `..._mac_x86.dmg`; **(2)** **January 2027**—if TI's own end-of-2026 deadline has passed with no native build, escalate this from a watch item to a decision input, since a missed self-imposed deadline is the first real evidence of slip; **(3)** **before the manuscript goes to press**—if still x86-only, the book needs a documented Rosetta-reinstall procedure for Mac students (Rosetta 2 is removed by the macOS 27 installer but can be reinstalled manually) and a note that Fall 2027's macOS 28 ends Intel-app execution outright. ST is already native, so this axis also carries a fallback: it is the one factor that could reasonably tip the choice to the H533RE late.
...
- [TI E2E—"Is CCS going to run on Apple silicon without depending on Rosetta?"](https://e2e.ti.com/support/processors-group/processors/f/processors-forum/1655979/ccstudio-is-ccs-going-to-run-on-apple-silicon-without-depending-on-rosetta)—TI's clearest and most current public commitment. **Posted July 2026**; a TI engineer replies "Yes. Work is already in progress for this. A version will be available before the end of the year, if not sooner." The "end of the year" is therefore end of **2026**, and the commitment is live, not lapsed.
...
- [TI E2E—"Plans for M1 Macs"](https://e2e.ti.com/support/tools/code-composer-studio-group/ccs/f/code-composer-studio-forum/983603/ccs-ccstudio-plans-for-m1-macs)—the oldest thread; TI states CCS is not officially supported on M1 and offers no native timeline. Included to show how TI's position has firmed up over time.
...
- [TI—CCS 21.0.0 download page](https://www.ti.com/tool/download/CCSTUDIO/21.0.0)—current release (June 15, 2026); the only macOS installer is `CCS_21.0.0.00014_mac_x86.dmg`. **This is the page to re-check each release**—a native build will appear here as an arm64 DMG.
...
**Caveat on the TI citations:** these are TI employee statements on the E2E support forums, not a formal published roadmap, and no TI document commits to a native arm64 CCS release date in print. The July 2026 statement is nonetheless recent, specific, and unexpired—treat it as a real commitment on a deadline worth tracking rather than as a broken promise.