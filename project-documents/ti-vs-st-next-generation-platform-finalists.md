...
Board comparison—internal—August 2026 (rev 9)

> **Revision note.** Rev 8 corrected a major error: earlier revisions claimed the STM32H533 lacked hardware encoder index and error detection, which was true of older STM32 families but false for this part. Rev 9 continues that audit and corrects four more things, two of which cut against TI: **(a)** TI makes no pinout-compatibility guarantee for controlCARDs, and the two candidate cards may not even be the same physical length; **(b)** the LaunchPad's USB isolation is defeated by default jumpers, so the isolation advantage claimed in rev 8 belongs only to the controlCARD; **(c)** neither the H533 nor the H563 has an on-chip op-amp or comparator—earlier revisions said the H533 did; **(d)** the Nucleo-H563ZI costs ~$39–44, not the ~$27 quoted in earlier revisions. Rev 9 also adds sections on encoder software usability, maximum count rate, and what a second core would and would not buy, and records the evaluation and rejection of the Nucleo-H563ZI as ST finalist.
...
| Debug on board | XDS110; isolator present but **defeated by default jumpers**; controlCARD isolated | ST-LINK/V3EC, **not isolated, not detachable** |
| Form factor | BoosterPack ×2; controlCARD edge card (see caveats) | Arduino Uno V3 + morpho; **no card-edge module exists** |
| macOS on Apple Silicon | x86 only; Rosetta 2 required; native promised by end of 2026 | **Native arm64** since Feb 2026 |
| Price (Aug 2026) | ~$35–36 | **~$23–26** |
| Replaceable-unit cost | ~$190–260 (controlCARD) or ~$35 (LaunchPad) | **~$26** |
...
Because the course builds its own carrier board, how the MCU attaches matters—and rev 9 corrects two claims made in rev 8.

**Correction 1: TI guarantees nothing about controlCARD pinout.** Rev 8 implied a family-wide compatibility promise. There isn't one. SPRUIJ6 says the dock supports controlCARDs conforming to the 180-pin and 120-pin connector *footprint*—footprint, not pinout, and "several," not "all." The pinout map is not even a numbered TI literature document; it ships inside C2000Ware.
...
**A physical-length ambiguity to settle before cutting copper.** SPRUJA7 describes the TMDSCNCD28P55X as a "120-pin HSEC8 Edge Card Interface" (and its C2000Ware pinout file is named `..._120cCARD_pinout`), while SPRUJ90 describes the TMDSCNCD28P65X as 180-pin. TI's product page for the P55x says "Standard 180-pin controlCARD HSEC interface"—but the F280049C has exactly the same contradiction, and a TI engineer on E2E confirmed that one is a 120-pin card. "180-pin controlCARD" appears to be an ecosystem label rather than a finger count. TI's own dock uses two connectors precisely to accommodate both lengths. **Verify against a physical card.**

TI has also broken this once per generation: DIMM100 → HSEC (requiring the TMDSADAP180TO100 adapter), and the F29H85x has now **abandoned controlCARD entirely** for a "controlSOM" using Samtec board-to-board connectors, with backward compatibility only through a separately purchased adapter. Calibrated recommendation if the group goes this route: design to pins 1–120 only, mirror TI's two-connector arrangement, and don't hard-assign meaning to analog pins in course material. Sell it internally as "reduces rework," not "future-proof."

---

**Correction 2: the LaunchPad's isolation is defeated by default.** Rev 8 credited TI with isolated debug generally. In fact SPRUJC0 is explicit that on the LaunchPad, "by default, both shunts are populated... meaning that the USB is NOT isolated from the XDS110 and F28P55x MCU regions." Pull both JP1 shunts and you get real galvanic isolation, but the board then has no power and must be fed 5 V externally—which the carrier could do for ~$0. Note that TI's LaunchPad product page pointedly omits the word "isolated," while the controlCARD pages say "Isolated on-board XDS110." **Only the controlCARD's isolation is inherent and not defeatable by a jumper.** The Nucleo has none and cannot be separated from its ST-LINK.

**Is the grounding issue real? Mostly not, at these voltages.** 12–24 V is SELV: no shock hazard and no hazardous path to a laptop. The most informative data point is that ST sells the X-NUCLEO-IHM07M1, an **8–48 V** three-phase motor shield designed to stack directly on a non-isolated Nucleo and be debugged over its non-isolated ST-LINK, and UM1943 contains no warning about isolation, ground loops, or PC connection at all. The genuine benefit of isolation here is debug-link robustness and ADC noise immunity against brushed-motor EMI—the symptom is "the debugger lost the target mid-demo," not smoke. Two free mitigations beat any hardware purchase: **specify a floating bench supply** (don't earth the motor return) and **have students run on battery during motor labs**. A two-prong laptop charger's Y-capacitors inject mains-frequency leakage into board ground, which is its own good teaching moment. Also worth knowing: an earthed bench-scope ground clip is a far more effective ground-loop generator than any USB cable. One purchasing gotcha—the common ADuM3160-based USB isolator dongles are full-speed only, and both the XDS110 and ST-LINK/V3EC are high-speed devices.

**ST has no card-edge module, for any STM32 family.** The "SOM-STM32xx" pages on st.com are partner pages for Emcraft—castellated solder-down modules on proprietary pinouts. STMod+ is a 20-pin peripheral-daughterboard connector that carries no MCU. The STM32MP SOMs are Cortex-A Linux parts. MikroElektronika's "MCU CARD for STM32" is the closest concept but is a proprietary 168-pin mezzanine with no debugger and no STM32H5 card. ST's model runs the other direction: the Nucleo is the fixed base and expansion boards stack on top, which is exactly what the X-NUCLEO-IHM16M1 motor driver does—and it includes a quadrature-encoder connector.

Three consequences. **Cost runs strongly ST's way**: a destroyed unit is ~$26 for a whole Nucleo with debugger included, versus $190–260 for a controlCARD. **Mechanical robustness runs TI's way**: edge insertion is polarized and rated for many cycles, where a 38+38-pin 0.1" header stack has high mating force and weak keying. And the morpho pinout is **not** guaranteed consistent across Nucleo-64 boards—ST publishes a separate table per board—so neither vendor offers a durable cross-board guarantee, TI's being stronger in practice but weaker than rev 8 claimed. If the group leans ST, consider terminating the carrier at **ST's 34-pin motor-control connector** (pinout in UM1970) rather than at the morpho headers, which puts a published, family-independent interface at the amplifier boundary.

---

## Would spending more buy anything? (up-market scan)

**TI, one tier up—LaunchPad F28P65X (~$66).** Dual 200 MHz C28x cores, ADCs with a 16-bit mode, two buffered DACs, six eQEP modules, more memory. Note what does *not* change: still exactly **one CLA**, shared across both cores. What the upgrade adds as a co-processor is a second full C28x core—arguably more useful than another CLA, since it runs arbitrary C with full peripheral access. The one loss is the NPU, exclusive to the P55x. Toolchain, driverlib, DCL, and book code carry over with a device-target change.
...
- **Price inverts the comparison.** Current distributor pricing is **$39–44** (low of $38.96; DigiKey around $43), not the ~$27 quoted in earlier revisions. That makes the ST board *more expensive than the $35 TI LaunchPad*, erasing ST's clearest advantage.
...
- **The one gap it would cure, it doesn't.** Cube H5 examples target the H563ZI, but there are still **zero encoder examples anywhere in CubeH5**, so the deficiency that actually matters persists.

**Decision: keep the Nucleo-H533RE as the ST finalist.** The H563ZI is better silicon on paper but a worse board for this course at a higher price, and its accelerators don't touch the argument deciding the comparison.
...
**Conclusion:** price was never the binding constraint, and spending more on either side does not buy what this book needs.

---

## Recommendation

**Lean: LaunchPad F28P55X—narrowly, and on software grounds.**
...
The case for ST is real and rev 9 strengthens two legs of it. ST is **cheaper on both counts**—$23–26 versus $35–36 for the board, and ~$26 versus $190–260 per replaceable unit if the controlCARD route is chosen. ST has a **native Apple Silicon toolchain today** where TI has a promise. ST has a second DAC channel and competitive encoder hardware. And two rev 8 arguments for TI have now been withdrawn: the controlCARD pinout guarantee (which doesn't exist) and the LaunchPad's isolation (which is defeated by default jumpers).

If the group weighs replacement cost heavily—plausible in a lab where students destroy hardware—or judges the Apple Silicon risk unacceptable, the H533RE is defensible and nothing in the labs would be blocked. But the lab-development cost is where the two genuinely diverge, and that cost lands on us rather than on the students.
...
- **Encoder count:** how many simultaneous encoders does the heaviest lab use? TI has 3 eQEP (6 on the F28P65x); the H533 has six encoder-capable timers, five with an index pin in the 64-pin package. If the controlCARD form factor is adopted, the HSEC180 standard exposes only two complete eQEP interfaces.
- **Encoder type and cabling:** single-ended or differential (RS-422)? At lab speeds neither MCU is close to its count-rate limit, so signal integrity beside motor PWM is the binding constraint, not bandwidth. Also fix the input-filter setting deliberately rather than by default—TI's maximum qualifier setting caps at ~3,445 RPM with a 512-line encoder.
...
- **Form factor and replacement economics:** LaunchPad + headers, controlCARD + HSEC socket, or Nucleo-as-shield? Replaceable-unit cost differs by roughly 7–10×, and neither vendor offers the durable cross-board pinout guarantee rev 8 assumed.
- **Debug isolation:** if the carrier hosts a motor amplifier, decide among (a) controlCARD with inherent isolation, (b) LaunchPad with JP1 shunts removed and 5 V supplied by the carrier, or (c) accept non-isolated debug with a floating bench supply and battery-powered laptops during motor labs. Option (b) is nearly free and should probably be the default if the LaunchPad is chosen.
...
- **To verify physically before committing to a carrier:** whether the TMDSCNCD28P55X is a 120-finger or 180-finger card (the documentation contradicts itself), and whether the F28P65x's DACA/DACC actually appear where SPRR478 suggests.
...
- [TMDSCNCD28P55X](https://www.ti.com/tool/TMDSCNCD28P55X) · [TMDSCNCD28P65X](https://www.ti.com/tool/TMDSCNCD28P65X) · [180-pin docking station guide SPRUIJ6](https://www.ti.com/lit/pdf/spruij6) · [controlCARD schematic SPRR478](https://www.ti.com/lit/pdf/sprr478)
- HSEC180 standard map—parsed from C2000Ware `boards/ExperimenterKits/DockingStation_HSEC_120or180pin/revF/180_HSEC8_DV_pinout_Rev_F.pdf`; full 180-pin table in project files as `HSEC180_controlCARD_standard_pinout.csv`
...
- [Octopart—NUCLEO-H533RE price/stock](https://octopart.com/part/stmicroelectronics/NUCLEO-H533RE) · [ST product longevity program](https://www.st.com/content/st_com/en/about/quality-and-reliability/product-longevity.html) · [ST press release 7 Mar 2023 (H563)](https://newsroom.st.com/media-center/press-item.html/p4519.html) · [ST blog 3 Apr 2024 (H533)](https://blog.st.com/stm32h5/)
...
**Method note.** Rev 8 and rev 9 corrections were verified against vendor datasheets, SVD-derived register definitions, CMSIS device headers, HAL sources, SDK file trees, and ST's pin database—not against community posts. The rev 1–7 encoder error came from trusting a forum thread about a different silicon generation. Where a claim could not be verified from a primary vendor document, it appears in the open questions rather than in the body. Two items remain flagged as unverified in the sub-research and are listed there: whether PA0 reaches the Zio headers on MB1404, and exact TMU per-instruction cycle counts (TI training material rather than SPRUHS1 Table 7-4).